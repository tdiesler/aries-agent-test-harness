...
public class DefaultEventHandler implements IEventHandler {
    ...
    public void handleEvent(String theirWalletId, String topic, String payload) {
        try {
            ...
            if (EventType.CONNECTIONS.topicEquals(topic)) {
                ...
                consumer = ev -> handleConnections(ev);
    ...
    public ConnectionRecord handleConnections(WebSocketEvent ev) throws Exception {
        ConnectionRecord item = ev.getPayload(ConnectionRecord.class);
        log.info("{}: [@{}] {} {} {}", ev.getThisWalletName(), ev.getTheirWalletName(), item.getTheirRole(), item.getState(), item);
        return item;
    }
    ...
    public V1CredentialExchange handleIssueCredentialV1(WebSocketEvent ev) throws Exception {
        ...
        log.info("{}: [@{}] {} {} {}", ev.getThisWalletName(), ev.getTheirWalletName(), item.getRole(), item.getState(), item);
        return item;
    }
    ...
    public RevocationEvent handleRevocation(WebSocketEvent ev) throws Exception {
        RevocationEvent item = ev.getPayload(RevocationEvent.class);
        log.info("{}: [@{}] {} {}", ev.getThisWalletName(), ev.getTheirWalletName(), item.getState(), item);
        return item;
    }

    public PresentationExchangeRecord handlePresentProofV1(WebSocketEvent ev) throws Exception {
